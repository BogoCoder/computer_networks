### Cálculos usando número de paquetes en total y tiempo en total.
...
class packet:
    def __init__(self,in_time,proc_time):
        ...
        self.start_time_in_system = 0
        self.end_time_in_system = 0
...
def increase_time(packet_index, t):
    ...
    for i in range(packet_index+1, num_packets):
        if packets[i].in_time < t:
            ...
            t -= packets[i].in_time
            packets[i].start_time_in_system = current_time + packets[i].in_time
            packets[i].in_time = 0
        else:
            packets[i].in_time -= t
            break
...
def simulation(in_rate, proc_rate):
    ...
    for i in range(num_packets):
        ...
        if packets[i].start_time_in_system == 0: packets[i].start_time_in_system = current_time
        if i in queue:
            queue.remove(i)
            if len(queue) == 0:
        ...
        packets[i].end_time_in_system = current_time
        for j in queue:
            packets[j].end_time_in_system = current_time
        int_N += (len(queue)+1)
        sum_T += (len(queue)+1)
        lambda_t = alpha/(i+1)
        T_bar = sum_T/alpha
...
print("Cálculos usando número de paquetes en total y tiempo en total.")
simulation(100,200)