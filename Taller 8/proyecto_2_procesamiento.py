### Cálculos usando número de paquetes en procesamiento y tiempo de procesamiento.
import numpy as np
...
def simulation(in_rate, proc_rate):
    ...
    for i in range(num_packets):
        ...
        packets[i].start_time_processing = current_time
        if i in queue:
            queue.remove(i)
            if len(queue) == 0:
        ...
        alpha += 1
        packets[i].end_time_processing = current_time
        int_N += 1
        sum_T += 1
        lambda_t = alpha/(i+1)
        T_bar = sum_T/alpha
...
print("Cálculos usando número de paquetes en procesamiento y tiempo de procesamiento.")
simulation(100,200)